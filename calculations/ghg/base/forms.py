...
from fingreen_web.models import (CollectionItem, GhgEmissionFactor,
...
class PredefinedFactorCalculationMethodForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['ghg_factor'].choices = [
            (factor.pk, f"{factor.factor_subtype_repr}{' > ' if factor.factor_subtype else ''}{factor.name}")  #pylint: disable=line-too-long
            for factor in self.initial['method'].factors.annotate(value_count=Count('values')).filter(value_count__gt=0) #pylint: disable=line-too-long
        ]

        self.helper.layout = Layout(
            Hidden('method', self.initial['method'].id), # field used in the view to preset form
...
class CustomFactorCalculationMethodForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['custom_factor_value'].widget.attrs['placeholder'] = _("Emission factor value")
